...
// Defining scenarios globally prevents re-declaration and logic errors.
const scenarios = {
    marketVolatilityEvent: [
        ...
        { text: "🎯 Strategy Planner assesses portfolio exposure, identifying 'at-risk' clients.", bubble: ["strategy-planner", "Identifying at-risk portfolios."], agent: "strategy-planner",
          ...
          challenge: "<h3>Data Integration & Security</h3><p>The agent needs real-time, secure access to sensitive client portfolio data across multiple legacy systems. A data breach or incorrect data connection would be catastrophic.</p><p><strong>Potential Solutions:</strong></p><ul><li>Leverage <strong>watsonx.data</strong> to provide a single, governed access point to client data across multiple databases and cloud environments, without moving the data itself.</li><li>Enforce strict Role-Based Access Control (RBAC) and data masking policies through <strong>watsonx.governance</strong> to ensure agents only see the data they are authorized to.</li></ul>"
        },
    ...
    sophisticatedFraudAttack: [
        { text: "‼️ FRAUD ALERT: Fraud Detection agent identifies a non-standard 'transaction slicing' pattern.", bubble: ["fraud-detection", "Anomalous transaction pattern!"], agent: "fraud-detection", type: 'event', handoffTo: "risk-analyst",
          ...
          challenge: "<h3>Quality & Performance</h3><p>The model must be incredibly accurate. A false negative means letting fraud through; a false positive means freezing legitimate customer accounts, causing immense frustration and brand damage.</p><p><strong>Potential Solutions:</strong></p><ul><li>Use <strong>watsonx.governance</strong> to continuously evaluate the model's accuracy, drift, and fairness against a 'golden dataset' of known fraud cases.</li><li>Manage the A/B testing and champion-challenger deployment of new model updates via <strong>watsonx.governance</strong> to ensure performance before a full rollout.</li></ul>"
        },
        { text: "COLLABORATIVE ACTION: Fraud Detection blocks IPs while the Compliance Officer freezes accounts.", agent: ['fraud-detection', 'compliance-officer'], bubble: ["fraud-detection", "IP blocked. Accounts frozen."],
          ...
          challenge: "<h3>Workflow Entanglement</h3><p>How do you ensure the agent's 'freeze account' command integrates flawlessly and securely with the bank's decades-old core banking system without causing unintended side effects?</p><p><strong>Potential Solutions:</strong></p><ul><li>Use <strong>watsonx Orchestrate</strong> to build the complex, multi-system workflow, sequencing actions like 'block IP' and 'freeze account' by connecting to the necessary security and core banking APIs.</li><li>Test this intricate workflow in a safe, sandboxed environment using a 'digital twin' of the core systems created with tools in <strong>watsonx.data</strong>.</li></ul>"
        },
    ...
    greenFundLaunch: [
        ...
        { text: "🗣️ Client Outreach agent identifies a target segment of clients and prepares a launch campaign.", bubble: ["client-outreach", "Targeted campaign ready."], agent: "client-outreach",
          ...
          challenge: "<h3>Data Privacy & Ethics</h3><p>How does the agent determine a client's 'interest in ethical investing'? This process must be transparent and avoid making inferences based on sensitive demographic data to prevent discriminatory targeting.</p><p><strong>Potential Solutions:</strong></p><ul><li>Use <strong>watsonx.governance</strong> to continuously monitor the agent's targeting logic for fairness and to detect and mitigate potential bias.</li><li>Restrict the agent's access to query only explicit, consent-given client data that is stored and managed within a governed <strong>watsonx.data</strong> lakehouse.</li></ul>"
        },
        { text: "EXECUTION: The fund is launched! The agents monitor initial investment flows.", agent: 'strategy-planner', bubble: ["strategy-planner", "The fund is live!"],
          ...
          challenge: "<h3>Cost Management</h3><p>Constant, real-time monitoring of multiple data streams can be computationally expensive. The cost of running the agent must not outweigh the value of the insights it generates.</p><p><strong>Potential Solutions:</strong></p><ul><li>Use <strong>Turbonomic</strong> and <strong>Apptio</strong> to gain a real-time, granular view of the compute and API costs of agent operations and correlate them to business value.</li><li>Build agents with smaller, cost-effective, specialized foundation models like the <strong>IBM Granite models</strong> via <strong>watsonx.ai</strong> for routine monitoring tasks to optimize spending.</li></ul>"
        }
    ],
...
// Runs ONCE when the page loads to create the buttons
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (firstButton) {
        const firstScenarioKey = Object.keys(scenarios).find(key => !key.startsWith('approve') && !key.startsWith('decline'));
        if (firstScenarioKey) {
